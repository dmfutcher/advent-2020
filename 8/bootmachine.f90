...
contains
    ...
    subroutine execute_operation(op, ctx)
        ...
        if (ctx%call_map(ctx%pc) /= 0) then
            ctx%halt = 2    ! halt=2 indicates unexpected (infinite-loop) halt
            return
        else
        ...
        if (ctx%pc > ctx%prog_len) then
            ctx%halt = 1    ! halt=1 indicates expected (program success) halt
        endif
    end subroutine

    function execute_program(prog) result(context)
